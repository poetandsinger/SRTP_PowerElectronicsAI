...
tags: [cs, ee, ai, machine-learning, design-automation, simulation, review]
sources:
  - ee/traction-inverter/research-synthesis-2025-2026
  - sources/cs/pe-gpt-2025-multimodal-pe-design
  - sources/cs/thermrag-2025-pe-thermal-agent
...
**Steelman against:** Every AI result cited here is from a paper where authors have an incentive to report positive results. The "22.2% improvement over human experts" from PE-GPT is on specific tasks (DAB, buck converter) — not general power electronics design. The "91% solve rate" from LLM+SPICE required 269 benchmark tasks; real design problems are not benchmark-curated.

**How it could be false:** Almost all results are on simplified benchmarks or specific converter topologies. No study tests AI on a novel, un-curated design problem. The LLM results in particular suffer from benchmark contamination — the training data may include solutions to the exact problems being tested. The SPICE+LLM solve rate improvement (15%→91%) may reflect prompt engineering of the benchmark, not general design capability.

**What would change my mind:** A blinded A/B test where AI-generated designs are compared against human expert designs on novel specifications neither has seen before. Independent reproduction of the PE-GPT 22.2% claim on traction inverter design specifically.

**Residual doubt:** AI-for-PE is a hot funding area. Every paper in this section is from authors and institutions that benefit from positive results. The convergence of findings is suggestive but not probative — we need adversarial testing, not more benchmarks.
