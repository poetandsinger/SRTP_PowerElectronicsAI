...
The **anchor reference design** the design cluster is built around: an 800V-class **SiC 2-level B6** traction inverter, 150 kW, driving an IPMSM. It pulls the spec, the key decisions, and the validation plan into one place. The sizing math is in [[power-electronics/traction-inverter/design-procedure]], the diagrams in [[power-electronics/traction-inverter/schematics]], the parts in [[power-electronics/traction-inverter/bom]].

**Citation convention:** `[NN]` → [[citations]]; `[T]` → training knowledge; **[derived]** → computed in [[power-electronics/traction-inverter/design-procedure]].

---

## Why This Anchor (industry relevance)

- **2L-B6 is >95% of production** BEV/PHEV traction inverters — the topology to master first [[power-electronics/traction-inverter/circuit-topologies]] §1, [28].
- **800V is the industry's migration direction:** Porsche→Hyundai E-GMP→VW PPE→BYD premium have moved to 800V for charging power and lower current losses [[power-electronics/traction-inverter/traction-inverter-index]], [28]. It is where **SiC pays off and forces 1200 V devices**, making it the design point where topology trade-offs (2L vs 3L-TNPC) actually matter [28][43].
- **It is the handoff's first PLECS model** on the critical path (2L-B6 SiC first). Anchoring here means the knowledge base feeds directly into the first validated model [80].

The **highest-*volume* SiC design today is 400 V** (Tesla Model 3/Y class) [31], [[power-electronics/traction-inverter/components]] §1.2 — captured as an alternative in the table below. We anchor on 800 V for *forward* relevance and because it is the harder, more instructive design point; the 400 V variant is a de-rating of the same procedure.
...
| Item | Value | Basis |
|------|-------|-------|
| Topology | 2-level B6 (six-switch VSI) | [[power-electronics/traction-inverter/circuit-topologies]] §1 |
...
| Modulation | SVPWM + DPWM at high load, overmod to six-step | [[power-electronics/traction-inverter/control-schemes]] §4 |
| Control | FOC, MTPA + field weakening, resolver | [[power-electronics/traction-inverter/control-how-to]] |
...
All **[derived]** in [[power-electronics/traction-inverter/design-procedure]]; these are algebra on `[T]` assumptions, not measurements [80].
...
2. **SiC, not IGBT:** unipolar, no tail current → 50–70% lower switching loss, decisive at the partial loads that dominate drive cycles [[power-electronics/traction-inverter/components]] §1.2, [28].
...
5. **Resolver + sensorless backup:** ASIL-D needs guaranteed position; sensorless alone can fail silently at zero speed [[power-electronics/traction-inverter/control-schemes]] §5, [48].
6. **ASC as safe state:** at speed, freewheel would pump the bus through body diodes; ASC bounds it with controlled braking [55], [[sources/power-electronics/pimpale-mahadik-2025-asc-discharge]].
...
| **3L-NPC** | 12 switches + 6 clamp diodes; half switch-voltage; NP balancing | industrial/rail; not cost-justified in auto [[power-electronics/traction-inverter/circuit-topologies]] §2 | [50][27] |
...
| **Dual inverter** | two smaller inverters, one per axle / open-winding | partial-load efficiency via load-splitting | [43], [[sources/power-electronics/sachs-etal-2025-single-dual-inverter]] |

The multilevel cases matter most **at 800 V**, which is exactly why this anchor is 800 V — it sets up the topology comparison the MAS is meant to reason about [28][43].
...
3. **The 3L-TNPC advantage rests on one preprint** [28] — see the Red Team in [[power-electronics/traction-inverter/circuit-topologies]]; the alternatives table inherits that single-source risk.
4. **Motor parameters `[T]`** propagate into every operating point; a real datasheet could shift currents and PF materially [[power-electronics/traction-inverter/control-how-to]] §8.
...
← [[power-electronics/traction-inverter/design-procedure]] | [[power-electronics/traction-inverter/schematics]] | [[power-electronics/traction-inverter/bom]] | [[power-electronics/traction-inverter/traction-inverter-index]] →
