...
A **component-class BOM** for the 800V-class SiC 2L-B6 reference design ([[power-electronics/traction-inverter/reference-design-2l-b6-sic-800v]]), organized by function. Each line gives the sizing driver, a **representative part/part-class**, and its citation. It is intentionally *class-level, not procurement-level*: exact MPNs, prices, and stock go stale fast and are better pulled live by the DigiKey/Nexar adapters noted in the handoff. Sizing rationale for every quantity is in [[power-electronics/traction-inverter/design-procedure]].

> **Priced version:** [[power-electronics/traction-inverter/bom-price-database]] carries real, dated distributor prices for these parts (and the distributor-vs-volume caveat). A real, buildable instantiation of this BOM — with vendor part numbers — is the Wolfspeed/TI CRD in [[power-electronics/traction-inverter/reference-design-wolfspeed-ti-300kw-800v]].

**Citation convention:** `[NN]` → [[citations]]; `[T]` → training knowledge; representative parts are drawn from the part-classes already catalogued in [[power-electronics/traction-inverter/components]] to keep this grounded. Part numbers are illustrative of the class and must be datasheet-checked before use (`[T]`), cf. schematic-vs-datasheet review [68].
...
|---|----------|-----|----------------------------------------|----------------------|------|
| 1.1 | Main switch | 6 (or 3 half-bridge modules) | 1200 V, ~450 A, hot Rds(on)~5 mΩ; §2 | SiC MOSFET module — Wolfspeed XM3 / onsemi VE-Trac Direct / Infineon CoolSiC HybridPACK class | [38][39][36], [[power-electronics/traction-inverter/components]] §1.2 |
...
Packaging note: direct-cooled pin-fin baseplate assumed (§Thermal); some OEMs integrate the DC-link cap onto the module to cut Lσ [[power-electronics/traction-inverter/components]] §5 [37].
...
|---|----------|-----|---------------|----------------------|------|
| 2.1 | Isolated gate-driver IC | 6 | ≥5 kV reinforced, ±10 A, desat <1.5 µs, Miller clamp; §5 | TI UCC21750 / Infineon 1EDI3021AS / onsemi NCD57000 class | [40][86], [[power-electronics/traction-inverter/components]] §2.2 |
...
| # | Function | Qty | Sizing driver | Representative class | Cite |
|---|----------|-----|---------------|----------------------|------|
| 3.1 | DC-link capacitor | 1 bank (~500 µF) | ~120 A rms ripple, ≥900 Vdc, low ESL; §4 | metallized-PP **film** module (TDK/CDE automotive DC-link class) | [41][84][90], [[power-electronics/traction-inverter/components]] §3 |
...
| # | Function | Qty | Sizing driver | Representative class | Cite |
|---|----------|-----|---------------|----------------------|------|
| 4.1 | Phase-current sensor | 2–3 | BW ≥50 kHz, isolated, ±1–2%; §6 | Allegro ACS37002 (Hall) / Infineon TLI4971 / TI INA253 + shunt class | [42], [[power-electronics/traction-inverter/components]] §4.2 |
...
| 4.4 | Rotor position | 1 | ±0.1°, ASIL-D; §6 | resolver + RDC, or inductive encoder | [48], [[power-electronics/traction-inverter/control-schemes]] §5.1 |
...
| # | Function | Qty | Sizing driver | Representative class | Cite |
|---|----------|-----|---------------|----------------------|------|
| 5.1 | Motor-control MCU | 1 | ASIL-D, motor-PWM, resolver IF, ≥6 ADC; §9 | Infineon AURIX TC3xx / TI C2000 / NXP S32K3 class | [T], [[power-electronics/traction-inverter/control-schemes]] §7.1 |
...
| # | Function | Qty | Sizing driver | Representative class | Cite |
|---|----------|-----|---------------|----------------------|------|
| 6.1 | Cold plate / heatsink | 1 | pin-fin water-glycol, 10–20 kW/L; §3 | direct-cooled pin-fin | [T], [[power-electronics/traction-inverter/components]] §6.1 |
...
Not a quote — a **relative** allocation to guide design effort, from public teardown-level commentary [T], [[power-electronics/traction-inverter/traction-inverter-index]]:
...
2. **Cost split is teardown-derived commentary**, not audited BOM cost; the SiC share especially moves with wafer economics [29] (a `motivated` source, [[power-electronics/traction-inverter/components]] Red Team).
...
← [[power-electronics/traction-inverter/design-procedure]] | [[power-electronics/traction-inverter/reference-design-2l-b6-sic-800v]] | [[power-electronics/traction-inverter/components]] →
