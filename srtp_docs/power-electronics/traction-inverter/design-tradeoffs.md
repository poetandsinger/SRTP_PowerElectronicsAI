...
No traction inverter maximizes every objective — each choice buys one metric by spending another. This chapter is the **compromise map**: the recurring decisions, what each one trades, and the rule of thumb for resolving it. It sits on top of [[power-electronics/traction-inverter/design-procedure]] (which sizes a *fixed* choice) and the reference designs (which are *points* on these trade curves).

**Citation convention:** `[NN]` → [[citations]]; `[T]` → training knowledge; `[derived]` → from [[power-electronics/traction-inverter/design-procedure]].
...
- **Buy with SiC:** 50–70% lower switching loss, higher fsw → smaller passives, far better **partial-load** efficiency (unipolar, no tail current) [[power-electronics/traction-inverter/components]] §1.2, [28].
...
- **Rule:** SiC wins at **high voltage/power and cycle-efficiency-critical** designs; IGBT still rational **<100 kW, 400 V, cost-first** [[power-electronics/traction-inverter/reference-design-nissan-leaf-400v-igbt]]; GaN not yet traction-ready (650 V, current limits) [[power-electronics/traction-inverter/components]] §1.3.
...
- **Rule:** 800 V for **new premium/high-power** platforms (the industry direction); 400 V for **cost-sensitive, highest-volume today** (Tesla-class) [[power-electronics/traction-inverter/reference-design-tesla-model3-400v-sic]] [94].
...
- **Rule:** pick the **lowest fsw that meets THD + NVH + passive-size targets**; SiC's low loss lets you afford 10–20 kHz where IGBT is stuck at 8–12 kHz [[power-electronics/traction-inverter/circuit-topologies]] §1.
...
- **Pay:** 12–18 switches vs 6, neutral-point balancing, gate-drive count and BOM/control complexity [[power-electronics/traction-inverter/circuit-topologies]] §5.
...
- **SVPWM:** best THD, full linear range to MI 0.907 [50], [[power-electronics/traction-inverter/control-schemes]] §4.2.
...
- **Rule:** pin-fin water-glycol is the default (10–20 kW/L); jet-impingement/double-sided when density-limited (Tesla/Lucid) [[power-electronics/traction-inverter/components]] §6.1, [94].
...
5. Size everything ([[power-electronics/traction-inverter/design-procedure]]), then **validate the compromise in PLECS** at 3 corners — the trade-offs only become real numbers there [80].

## Red Team

**Steelman against:** This chapter states trade directions as if they were laws, but the crossover points are design-, cost-, and year-specific. "SiC wins above X kW" has no fixed X — it moves with SiC $/area [29], cooling, and drive cycle. The 3L-TNPC advantage rests on a single preprint [28] (see [[power-electronics/traction-inverter/circuit-topologies]] Red Team). And "lowest fsw that meets targets" hides that the targets themselves trade against each other.
...
4. **No cost model** — "$/kW" directions are qualitative; real BOM cost needs [[power-electronics/traction-inverter/bom-price-database]] + volume quotes.
...
← [[power-electronics/traction-inverter/design-procedure]] | [[power-electronics/traction-inverter/reference-designs-index]] | [[power-electronics/traction-inverter/circuit-topologies]] →
