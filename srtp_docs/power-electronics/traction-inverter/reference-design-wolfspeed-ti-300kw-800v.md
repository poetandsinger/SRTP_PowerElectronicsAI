...
A **real, published, buildable** 800 V / 300 kW SiC traction-inverter reference design co-developed by Wolfspeed and Texas Instruments — vendor part numbers, schematic, and BOM are public [91]. It is the closest thing to a "known-good answer" for the anchor class in [[power-electronics/traction-inverter/reference-design-2l-b6-sic-800v]], and the best template for the PLECS 2L-B6 SiC model on the critical path.
...
This confirms the design-procedure order of magnitude: an 800 V SiC 2L inverter reaches >98% efficiency at ~30 kW/L with 1200 V modules — consistent with the estimates in [[power-electronics/traction-inverter/design-procedure]] §3 [derived] and the SiC-2L ranges in [[power-electronics/traction-inverter/traction-inverter-index]] [28].
...
| DC-link | film capacitor bank | 800 V bus, ripple/ESL duty | — (see [[power-electronics/traction-inverter/bom-price-database]]) | [41][90] |

**Reality check on cost:** three CAB450 modules at single-unit distributor price ≈ **$2,700** — yet the entire Tesla Model 3 inverter cost ~**$810** in 2018 at automotive volume [94]. Distributor single-unit ≠ OEM volume pricing by roughly an order of magnitude; the price database says so explicitly [[power-electronics/traction-inverter/bom-price-database]].

---

## How It Maps to Our Design Notes

- **Module = 1200 V/450 A**, exactly the class the sizing procedure lands on for an 800 V bus [[power-electronics/traction-inverter/design-procedure]] §2 [92].
- **Gate driver = UCC5880-Q1** implements the desat / adjustable-drive / functional-safety primitives described in [[power-electronics/traction-inverter/schematics]] §4 and [[power-electronics/traction-inverter/components]] §2 [93].
- **Low power-loop inductance (5.3 nH)** meets the `Lσ < 10–15 nH` busbar budget [[power-electronics/traction-inverter/design-procedure]] §8 [91].
...
← [[power-electronics/traction-inverter/reference-designs-index]] | [[power-electronics/traction-inverter/reference-design-2l-b6-sic-800v]] | [[power-electronics/traction-inverter/bom-price-database]] →
