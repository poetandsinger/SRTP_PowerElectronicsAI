...
| Item | Value | Basis |
|------|-------|-------|
| Topology | 2-level B6 (6-switch VSI) | >95% of production, [[power-electronics/traction-inverter/circuit-topologies]] §1 |
| DC bus | **355 V nom** (280–420 V), 450 V clamp | 96S NMC family pack `[T]` |
| Motor | IPMSM: `Pp`=4, `Rs`=15 mΩ, `Ld`=0.15 mH, `Lq`=0.30 mH (saliency 2.0), `λPM`=0.075 Wb | scaled from [[power-electronics/traction-inverter/machine-and-load]] §3 [47][50] |
...
| Cooling | pin-fin water-glycol, 65 °C inlet | [[power-electronics/traction-inverter/thermal-design]] |

## 3. Topology, Device & Loss Model

**2L-B6 SiC**, benchmarked against a **750 V Si-IGBT** comparator. Device loss params are datasheet-class typicals `[T]` (per switch): SiC `Rds(on,150°C)`≈4.5 mΩ, `Eon+Eoff@400V/400A`≈9 mJ; IGBT `Vce0`≈0.9 V + `Rce`≈2.5 mΩ, diode 1.0 V + 1.8 mΩ, `Eon+Eoff+Err`≈32 mJ [25][122]. SiC at 12 kHz, IGBT at 8 kHz (its practical ceiling). **750 V not 650 V:** worst-case 420 V bus + `Lσ·di/dt` overshoot → ~72% utilization with cosmic-ray margin [[power-electronics/traction-inverter/protection-and-safety]] §1 [121].

## 4. Results — the Model Run  `[model]`

**Efficiency at 3 DC corners** (peak-power operating point; conduction dominates at low-line as [[power-electronics/traction-inverter/worked-example-400v-150kw]] predicts):
...
Per [[power-electronics/traction-inverter/control-how-to]]: FOC + MTPA + field-weakening, SVPWM, resolver, ASC safe state. **IMC current-loop gains** at BW = 2π·1.5 kHz [derived §3, control-how-to §3]: `Kp_d`=1.41, `Kp_q`=2.83 V/A, `Ki_d=Ki_q`=141 V/(A·s). 12 kHz → 83 µs PWM, double-update 42 µs, current-loop BW ~1/8 of update. MTPA runs `id<0` for reluctance torque (saliency 2.0); field-weakening from ~4500 rpm base to 12 000 rpm. Safety: resolver-fault / torque-mismatch → **ASC** (bus would otherwise pump via body diodes at speed) [55], [[power-electronics/traction-inverter/protection-and-safety]] §5.

## 6. BOM — the actual parts and why

Class-level method in [[power-electronics/traction-inverter/bom]]; here are the **specific parts chosen for this design**, each tied to the computed sizing driver. Part numbers are representative-class and must be datasheet-checked `[T]`, per vault convention.
...
PLECS Standalone is **licensed and driveable via XML-RPC** here (`plecs.load/set/simulate`) — this **clears the vault's flagged "PLECS license check"** blocker [[README]]. The `permanent_magnet_synchronous_machine` FOC demo was **retargeted to this machine** (`Rs`=15 mΩ, salient `Ld/Lq`, `λ`=0.075, 355 V) and **simulated to completion** (`worked-designs/family-car-400v-sic/pmsm_mycar.plecs`) [58][78][72]. Quantitative readback needs top-level outports on torque/current/loss — the documented next step ([[power-electronics/traction-inverter/simulation-and-validation]] §4). Until then §4 numbers are the analytic model, equivalent to PLECS's *averaged* thermal-loss layer, pending its *switching-resolved* confirmation.

## 8. The Report — Compromises

- **400 V, not 800 V:** chosen for a *common* family car — mature 650/750 V devices, cheaper isolation, huge ecosystem [94]. Cost: 2× current → conduction-dominated loss, bigger cap/busbar, ~0.5 pt lower peak η than an 800 V build [[power-electronics/traction-inverter/worked-example-400v-150kw]].
...
- **12 kHz:** above cabin-audible harshness, low enough for loss/EMI; SiC could clock higher but EMI/switching-loss rises [[power-electronics/traction-inverter/emi-emc-design]] §2.
...
4. **Motor params `[T]`** (saturation, temperature) shift MTPA and currents ±20–30% [[power-electronics/traction-inverter/machine-and-load]] Red Team.
...
← [[power-electronics/traction-inverter/worked-example-400v-150kw]] | [[power-electronics/traction-inverter/findings-family-car-design-by-doing]] | [[power-electronics/traction-inverter/reference-designs-index]] →
