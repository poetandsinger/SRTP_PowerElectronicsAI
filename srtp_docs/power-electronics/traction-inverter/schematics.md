...
Connectivity/block **schematics** for a 2L-B6 SiC traction inverter, drawn in Mermaid so they render in Obsidian and stay diff-able for RAG. They anchor the reference design in [[power-electronics/traction-inverter/reference-design-2l-b6-sic-800v]] and the sizing math in [[power-electronics/traction-inverter/design-procedure]].

**Scope limit (be honest):** Mermaid is a graph/flow renderer, not a schematic-capture tool. These diagrams show **what connects to what and the signal/power flow** — not exact analog symbols, net names, or component footprints. The transistor-level switching arrangement (anti-parallel body diode, complementary switch pairs, switching states) is in [[power-electronics/traction-inverter/circuit-topologies]] as ASCII circuits. For a real board you capture these in KiCad/Altium and datasheet-check the connections (cf. [[sources/ai-agents/drcy-2026-allspice-mas-review]] [68]); these are the intent, not the netlist.

**Citation convention (this note and the whole design cluster):** `[NN]` → numbered entry in [[citations]]; `[T]` → training knowledge, not verified against a primary source; **derived values** (currents, capacitance, loss, Lσ budget) are computed in [[power-electronics/traction-inverter/design-procedure]] and cited there — the diagram labels reproduce those results, so read the referenced §. No value here is meant to stand without one of these markers.
...
The **commutation loop** is `DC+ → Q_H → Q_L → DC- → Cdc → DC+`. Its stray inductance `Lσ` sets turn-off overshoot (`V = Lσ·di/dt`) and ringing — target `Lσ < 10–15 nH` for SiC [derived in [[power-electronics/traction-inverter/design-procedure]] §8; loss/thermal basis [25], general PE [50]]. The Kelvin-source requirement for fast SiC switching is a gate-driver design rule [40][50].
...
Two-sensor vs three-sensor: two phase currents suffice (`Ia+Ib+Ic=0`) [50], but three give redundancy for ASIL and open-phase detection [85]. Sensor part-classes in [[power-electronics/traction-inverter/components]] §4 and [42].

---

## 7. Control Signal Chain (FOC → gate)

The compute path executed every current-loop tick (25–50 µs). Math detail in [[power-electronics/traction-inverter/control-schemes]] and [[power-electronics/traction-inverter/control-how-to]].
...
On an ASIL fault (resolver loss, torque mismatch) [85], the safe state for a PMSM is **all low-side ON** — shorts the machine, back-EMF circulates as braking current instead of pumping the DC-link through body diodes (freewheel would overvolt the bus at high speed) [55][50]. The residual braking-torque figure (~0.1–0.3 pu) and the freewheel-vs-ASC trade are from [[sources/power-electronics/pimpale-mahadik-2025-asc-discharge]] [55] and [[power-electronics/traction-inverter/control-schemes]] §6.4.
...
**What would change my mind:** A captured schematic (KiCad/Altium) reviewed against device datasheets — exactly the DRCY-style check in [[sources/ai-agents/drcy-2026-allspice-mas-review]] — and a PLECS/LTspice model reproducing turn-off overshoot within the `Lσ` budget.
...
← [[power-electronics/traction-inverter/circuit-topologies]] | [[power-electronics/traction-inverter/reference-design-2l-b6-sic-800v]] | [[power-electronics/traction-inverter/design-procedure]] →
