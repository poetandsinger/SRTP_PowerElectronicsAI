...
The physical build: module internals, the laminated busbar, gate/power-loop layout, creepage/clearance, and enclosure. These are where the `Lσ<15 nH`, thermal, and EMI targets are actually met or lost — the design-procedure treats them as budgets ([[power-electronics/traction-inverter/design-procedure]] §8); this chapter is how you hit them.
...
Power-cycling lifetime is set by CTE-mismatch fatigue at these interfaces (wire-bond lift-off, solder/sinter cracking) — the AQG 324 wear-out metric (+20% Rth / +5% Vf) [88], [[power-electronics/traction-inverter/protection-and-safety]] §7.

## 2. DC-Link Busbar — Meeting the `Lσ` Budget

- **Target `Lσ < 10–15 nH`** for SiC: overshoot `V=Lσ·di/dt`, ring `f=1/(2π√(Lσ·Coss))` [25], [[power-electronics/traction-inverter/components]] §5. Reference designs hit **5.3 nH power loop / 6.7 nH module** [91].
...
- Cap-integrated-into-module (Tesla-style) is the extreme case of loop minimization [37], [[power-electronics/traction-inverter/components]] §5.

## 3. Gate-Loop & Power-Loop Layout (PCB)

- **Kelvin source** (separate gate-return from power-source): mandatory for SiC — keeps power-loop `di/dt` out of the gate loop, or switching speed collapses and false-turn-on margin vanishes [107], [[power-electronics/traction-inverter/gate-driver-design]] §1, [[power-electronics/traction-inverter/schematics]] §3.
...
- **Separate power and signal grounds**, single-point HF bond; keep sense (current/NTC) routing away from the switching node [117], [[power-electronics/traction-inverter/emi-emc-design]] §6.
...
- **Cooling integration:** cold plate is structural — module bolts to it through the TIM (or is direct-cooled); coolant 65 °C WEG, ~10 L/min [99], [[power-electronics/traction-inverter/thermal-design]] §4.
- **Environment:** −40 to +85 °C ambient, severe vibration/shock, salt — busbar/connector mechanical fatigue and TIM pump-out are lifetime risks [T], [[power-electronics/traction-inverter/what-is-a-traction-inverter]] §7.
...
← [[power-electronics/traction-inverter/thermal-design]] | [[power-electronics/traction-inverter/emi-emc-design]] | [[power-electronics/traction-inverter/components]] →
