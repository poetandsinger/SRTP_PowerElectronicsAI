...
A **priced** version of the component-class BOM ([[power-electronics/traction-inverter/bom]]) for the SiC 2L-B6 reference designs. Each line carries a representative part, a **single-unit distributor price** with its source and date, and a note where the price is an estimate. It exists so an LLM designing a board has real cost anchors — with the critical caveat below.
...
The switch dominates either way — which is why device selection and loss/thermal design (how much silicon you must buy) is where cost is won [28][43], [[power-electronics/traction-inverter/bom]] §7.
...
← [[power-electronics/traction-inverter/bom]] | [[power-electronics/traction-inverter/reference-design-wolfspeed-ti-300kw-800v]] →
