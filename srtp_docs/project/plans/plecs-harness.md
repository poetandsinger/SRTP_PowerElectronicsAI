...
> Topic of [[project/plans/ai-agent-mas-plan]]. PLECS is the sole simulator and source of evidence (invariants #2, #6). Fills gap G-E (summarizer is a promise, not a contract) and G-F (model workstream is a table, not a procedure). Research basis: [[ai-agents/harness/plecs-integration]], [[sources/ai-agents/plecs-xmlrpc-scripting-interface]], [[sources/ai-agents/plecs-ai-agent-integration-ordonez]], PE-MAS `plecs-mcp`.
...
- **License check — do this first, before any harness work.** Confirm the installed PLECS license permits headless/scripted batch use. This is a hard blocker; verify it day one ([[project/plans/evaluation-and-benchmark]] risks).
- **Agent-facing tools:** `plecs_simulate`, `plecs_sweep` (list-of-optStructs, parallel), `plecs_set_param`, `plecs_analyze` (native AC/steady-state), `summarize_result`. The optimizer ([[project/plans/design-loop]] §2) calls `plecs_sweep` one generation at a time.
...
Per PE-MAS's honest `model_registry.json`, **validated per-topology models are the bottleneck, not the agent.** A model is only usable as evidence once it passes this procedure:
...
← [[project/plans/ai-agent-mas-plan]] | [[project/plans/design-loop]] | [[project/plans/guardrails-and-evidence]] | [[ai-agents/harness/plecs-integration]]
