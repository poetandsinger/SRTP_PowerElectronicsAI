...
> Topic of [[project/plans/ai-agent-mas-plan]]. Fills G-A (missing optimizer), G-B (black-box design), G-H (unnamed evaluator-optimizer). Research basis: [[ai-agents/design-loop-architecture]], [[ai-agents/agentic-workflow-patterns]].
...
**Default: Differential Evolution or PSO over PLECS `list-of-optStructs`.** One optimizer generation = one batch call PLECS distributes across CPU cores ([[sources/ai-agents/plecs-xmlrpc-scripting-interface]]) — the parallelism is free.
...
- **Mixed space:** module choice is *categorical* (handled in ② by the Designer/Planner); the optimizer searches the *continuous* residual (fsw, deadtime, Rg, C_dc, MI, loop gains). Measure the free-param dimensionality of one real spec before committing DE vs BO ([[ai-agents/design-loop-architecture]] red-team).
...
Name it plainly ([[ai-agents/agentic-workflow-patterns]] §3):
- **Rubric** = the evidence gates ([[project/plans/guardrails-and-evidence]]) — explicit, measurable.
...
- **Trace compaction:** each iteration compacts its reasoning trace ([[project/plans/memory]]) so the loop does not reinflate context (AnalogSAGE Compression Module).
...
Explicit mapping, **not** learned routing — enough at this depth ([[ai-agents/agentic-workflow-patterns]] §4).
...
← [[project/plans/ai-agent-mas-plan]] | [[project/plans/architecture]] | [[project/plans/guardrails-and-evidence]] | [[ai-agents/design-loop-architecture]]
