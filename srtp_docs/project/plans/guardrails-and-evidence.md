...
> Topic of [[project/plans/ai-agent-mas-plan]]. These are the **evaluator's rubric** for the evaluator-optimizer loop ([[project/plans/design-loop]] §3). Adopted from PE-MAS, re-tuned to traction. Domain grounding: [[power-electronics/traction-inverter/protection-and-safety]], [[power-electronics/traction-inverter/standards-and-compliance]].

## 1. Domain guardrails REVISE, NEED [[standards-and-compliance]] ( hard rules — system-prompt + post-sim hooks)

Non-overridable; injected as system-prompt constraints **and** enforced as post-simulation hooks so a violating result never enters agent context (Claude Code PostToolUse pattern — [[ai-agents/harness/architecture-patterns]] P3):
...
| **Citation** | every design claim cites a **retrievable** source ([[project/plans/knowledge-rag]]) | Validator |
...
**Hard rule:** no gate may close as "PLECS-backed" against an **unvalidated** model ([[project/plans/plecs-harness]] §3).
...
- Gate failures return to the **narrowest** responsible stage via the `iterate` router ([[project/plans/design-loop]] §4).

## 4. Anti-Goodhart caution

Gates are metrics; metrics get gamed. Two guards: (a) the **citation gate** ties design choices to external evidence, not just to passing numbers; (b) **human signoff** on the review package is the backstop the optimizer cannot optimize away. Watch for designs that pass all gates but are physically odd — the red-team's "design by checklist" risk ([[ai-agents/traction-inverter-mas-integration]] §7).

← [[project/plans/ai-agent-mas-plan]] | [[project/plans/design-loop]] | [[project/plans/plecs-harness]] | [[power-electronics/traction-inverter/protection-and-safety]]
