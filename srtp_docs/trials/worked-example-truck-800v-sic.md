...
A worked example whose **workflow is the point**: a Class-8 long-haul e-truck inverter is **not** sized by 0–100 peak (that's the family car, [[worked-example-family-car-400v-sic]]) — it is sized by **power-cycling life over ~1 M km under sustained load**. The workflow is the physics-of-failure chain from [[reliability-and-lifetime]]: mission → loss → Foster `Tj(t)` → rainflow → `Nf` → Miner `LC`. Runnable model: `worked-designs/truck-800v-sic/truck_lifetime.py`.
...
**Life ≈ 4.4 M km — passes the 1 M km target ~4×** [model]. **Sizing sweep:** because `Nf ∝ ΔTj⁻⁵`, a **+15% silicon/cooling** upsize (Tj,max 140→125 °C) moves *predicted* life ~100×. Life is therefore **hyper-sensitive to ΔTj** — design to a **Tj/ΔTj ceiling**, not to an absolute km number.
...
- **Process contrast:** where the family car's workflow converged on *efficiency at 3 voltage corners*, this one converges on *ΔTj over a mission* — a different model (rainflow+Miner), a different binding constraint (fatigue), a different sizing rule (Tj ceiling). See the 4-way comparison in [[design-by-doing-observed-workflow]].
...
← [[worked-example-family-car-400v-sic]] | [[reliability-and-lifetime]] | [[worked-example-performance-800v-sic]] →
