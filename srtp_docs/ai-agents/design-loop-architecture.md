...
Across every 2026 PE/analog design agent that **converges to numbers**, the design stage is not one step — it is three, ending in an **explicit numerical optimizer** over the simulator. The LLM chooses *structure*; a metaheuristic/Bayesian optimizer chooses *values*. This refines "LLM proposes, simulation disposes" into **"LLM proposes structure, optimizer disposes numbers, PLECS is truth."**
...
|---|---|---|---|---|
| **AnalogSAGE** (2025, analog IC) [[sources/ai-agents/analogsage-2025-self-evolving-analog-mas]] | Topology Selection Agent (50-cand DB + memory) | Refinement Agent (bias/compensation) | **Bayesian opt + ngspice** | spec check, ≤3 iters |
| **PHIA/LP-COMDA** (AAAI 2026, DAB) [[sources/ai-agents/phia-lpcomda-2026-physics-informed-pe-agent]] | modulation-strategy select | (in planner) | **PSO/DE/GA over PINN surrogate** | physics-informed surrogate |
| **PE-GPT** (IEEE TIE 2025) [[sources/ai-agents/pe-gpt-2025-multimodal-pe-design]] | Model-Zoo topology reasoning | RAG-guided | **metaheuristic over Model Zoo/Sim Repo** | sim repository |
| **Multi-Agent LLM Control** (2026, boost) [[sources/ai-agents/multi-agent-llm-control-2026-pe]] | objective→control structure | agent-decomposed | **PSO/GA** for gains | simulation, <2% err |
...
Without ③, the only convergence route is LLM-picks → PLECS-validates → LLM-re-picks: token-expensive descent by LLM judgement, the failure Ordonez reports ([[sources/ai-agents/plecs-ai-agent-integration-ordonez]]) and the reason the cited systems use a real optimizer.
...
Objective = weighted evidence-gate score (η, Tj margin, THD, cost) from the ~36-number summary; the optimizer sees summaries, never waveforms. DE/PSO map onto PLECS `list-of-optStructs` batch ([[sources/ai-agents/plecs-xmlrpc-scripting-interface]]) — parallelism is free. **Start with grid** on 2L-B6; adopt DE/BO when topology breadth enlarges the space. Surrogates stay a deferred *search accelerator* (optimize over surrogate, verify top-k on PLECS), never an evidence source (PHIA shows ~10-sample bootstrap is feasible if PLECS runtime later dominates).

## 4. Maps to existing agents — no new agents

- **Planner** owns ① (topology + control, RAG-grounded) — the LLM value-add ([[audits/ai-agent-docs-audit-2026-07-17]] §7).
...
Keeps the 3-agent core intact (AgentSlimming). Named-pattern basis and exclusions: [[ai-agents/agentic-workflow-patterns]].

## 5. Decision

Adopt the three stages as the internal structure of PLAN→DESIGN; add an explicit optimizer as ③ (grid → DE/PSO → BO by dimensionality); route fail edges to the narrowest stage. Surrogates deferred. Folded into [[project/plans/design-loop]].
...
← [[ai-agents/agentic-workflow-patterns]] | [[ai-agents/traction-inverter-mas-integration]] | [[project/plans/design-loop]] | [[ai-agents/design-by-doing-observed-workflow]] | [[sources/ai-agents/analogsage-2025-self-evolving-analog-mas]]
