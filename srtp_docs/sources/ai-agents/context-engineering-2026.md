...
**Why it matters for SRTP:** the plan's core cost rule — *summarize before the LLM* ([[sources/ai-agents/plecs-ai-agent-integration-ordonez]]) — is one tactic inside a named discipline, **context engineering**. The discipline gives SRTP a checklist for the memory/summarization design instead of one ad-hoc rule, and maps onto the [[ai-agent-mas-plan|plan]].
...
- **Isolate** — separate context per sub-task/agent so one agent's clutter doesn't pollute another's reasoning. *SRTP:* per-agent context (Planner sees papers; Validator sees numbers, not prose) — the [[ai-agents/harness/architecture-patterns|granular-context]] pattern.
...
## SRTP implications
- The plan's "SQLite + LanceDB, kept simple" is defensible, but it should be *named against this taxonomy* so gaps are visible: SRTP has write/offload + select/retrieve + a domain compaction, but **isolation** (per-agent context scoping) and **procedural memory** (iteration playbooks) are under-specified. See [[project/plans/memory]].
