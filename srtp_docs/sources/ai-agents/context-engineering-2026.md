...
- **Isolate** — separate context per sub-task/agent so one agent's clutter doesn't pollute another's reasoning. *SRTP:* per-agent context (Planner sees papers; Validator sees numbers, not prose) — the [[architecture-patterns|granular-context]] pattern.
...
## SRTP implications
- The plan's "SQLite + LanceDB, kept simple" is defensible, but it should be *named against this taxonomy* so gaps are visible: SRTP has write/offload + select/retrieve + a domain compaction, but **isolation** (per-agent context scoping) and **procedural memory** (iteration playbooks) are under-specified. See [[memory]].
...
← [[harness-index]] | [[agentic-workflow-patterns]] | [[memory]]
