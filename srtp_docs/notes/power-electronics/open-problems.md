...
The traction inverter is a mature field, but several design tensions remain unresolved. These questions guide the project's work. Each covers the trade-off, current industry practice, and what remains open.
...
**Open issue:** The 0.67 kWh/100 km gain from Sachs & Neuburger (2025) is worth chasing only if the extra 30% SiC chip area costs less than the lifetime fuel savings. The crossover depends on SiC wafer cost and the drive cycle — no universal answer. Sachs et al. (2025) [43] go further with an optimization-based comparison of single and dual traction-inverter architectures, explicitly trading chip area against partial-load system efficiency — one of the few studies that moves beyond topology-by-topology comparison to system-level optimization.
...
**Open issue:** The optimal switching speed is not a fixed number but a system-level compromise between efficiency, reliability, and filter/cooling cost — no simple design rule.
...
- Online methods (extremum-seeking, signal injection, RLS) remove calibration and adapt to parameter drift. Zuo et al. (2024) [45] show a recursive-least-squares dual-control approach improves dynamic performance over extremum seeking, at the cost of added estimator complexity and convergence sensitivity during transients [T].
