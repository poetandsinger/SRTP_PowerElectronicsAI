...
| Parameter search → converged values | inner optimizer | **missing → [[design-loop-architecture]]** |

SRTP is a **prompt-chain workflow with an evaluator-optimizer gate** — the right shape. The design-loop's missing numerical optimizer is treated in [[design-loop-architecture]]; this note covers the pattern discipline around it.

## 3. Evaluator–optimizer as first-class

The `VALIDATE --fail--> re-plan/re-design` edge is an evaluator–optimizer loop. It earns its keep only with a **separate judge** and **explicit criteria** (DRCY — [[drcy-2026-allspice-mas-review]]); otherwise "iterate" is vibes. SRTP already has the materials: evidence gates = rubric, k=3 consensus = judge, Planner/Designer = optimizer. The obligations:
...
| Deep recursion (subagents spawning subagents) | one delegation level covers the workflow ([[multi-agent-synthesis]] §1) | a subtask needs its own team |
...
Prompt-chain + evaluator-optimizer + tool-use, flat delegation, explicit routing. Name the evaluator-optimizer loop (§3); add the numerical optimizer ([[design-loop-architecture]]); defer the rest against the triggers above.
