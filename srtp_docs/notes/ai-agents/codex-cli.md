...
tags: [ai-agents, codex-cli, architecture]
---

## Overview

Codex CLI is OpenAI's **autonomous coding agent** for the terminal. It runs inside git repositories to implement features, fix bugs, refactor code, and review PRs. It requires an OpenAI API key or Codex OAuth credentials.

Codex is the **most constrained** of the coding agents — the fewest extension points, tightly coupled to OpenAI's ecosystem.
...
Codex **refuses to run outside a git repository** — a deliberate safety/accountability measure so all changes are tracked in git.
...
Codex CLI is the least suitable harness for research: a straightforward coding agent with no extension points, no multi-agent support, and no non-code task support. Using it for power electronics research would require:
