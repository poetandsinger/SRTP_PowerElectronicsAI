...
Claude Code and OpenCode are excellent coding agents but lack the operational infrastructure research workflows demand. Coding-only agents (Codex CLI, Aider, Cline) are architecturally designed for code editing, not scientific simulation workflows.
...
No harness provides these natively — they must be custom-built regardless of choice:
...
- **Fault tolerance:** Only LangGraph has built-in checkpointing for long-running workflows — critical for multi-hour MATLAB simulations that may fail mid-run.
- **Provider lock-in:** Claude Code is Claude-only, Codex OpenAI-only. Hermes, OpenCode, LangGraph, and CrewAI are provider-agnostic.
...
Qualitative strength per dimension (● strong · ◐ medium · ○ weak). Rendered as a matrix rather than a mermaid radar chart, which Obsidian does not reliably support.
