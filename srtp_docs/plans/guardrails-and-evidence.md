...
> Topic of [[ai-agent-mas-plan]]. These are the **evaluator's rubric** for the evaluator-optimizer loop ([[design-loop]] §3). Adopted from PE-MAS, re-tuned to traction. Domain grounding: [[protection-and-safety]], [[standards-and-compliance]].

## 1. Domain guardrails REVISE, NEED [[standards-and-compliance]] ( hard rules — system-prompt + post-sim hooks)

Non-overridable; injected as system-prompt constraints **and** enforced as post-simulation hooks so a violating result never enters agent context (Claude Code PostToolUse pattern — [[architecture-patterns]] P3):
...
| **Citation** | every design claim cites a **retrievable** source ([[knowledge-rag]]) | Validator |
...
**Hard rule:** no gate may close as "PLECS-backed" against an **unvalidated** model ([[plecs-harness]] §3).
...
- Gate failures return to the **narrowest** responsible stage via the `iterate` router ([[design-loop]] §4).
...
← [[ai-agent-mas-plan]] | [[design-loop]] | [[plecs-harness]] | [[protection-and-safety]]
