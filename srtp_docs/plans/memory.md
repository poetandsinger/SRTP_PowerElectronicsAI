...
> Topic of [[ai-agent-mas-plan]]. Fills gap G-D (memory named as a choice, not designed). Framed against the 2026 context-engineering taxonomy ([[context-engineering-2026]]); depth ceiling from AnalogSAGE ([[analogsage-2025-self-evolving-analog-mas]]); baseline from PE-MAS lifelong memory.

## 1. Four memory roles (name them, then decide depth)

| Role | Holds | Store | Status |
|---|---|---|---|
| **Working** | the live LangGraph state ([[architecture]] §4) | context window | built-in |
...
| **Semantic** | the paper corpus / domain facts | LanceDB (shared w/ RAG) | **build** ([[knowledge-rag]]) |
...
- **Compress / compact:** two compactions — the ~36-number PLECS summary (domain) **and** the per-iteration reasoning-trace compaction (AnalogSAGE Compression Module). Without the second, a long re-design loop reinflates context and the cost story breaks ([[design-loop]] §3).
...
← [[ai-agent-mas-plan]] | [[knowledge-rag]] | [[context-engineering-2026]]
