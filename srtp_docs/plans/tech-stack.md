...
> Topic of [[ai-agent-mas-plan]]. Consolidates the technology decisions, refreshed to mid-2026 currency ([[agent-frameworks-2026-currency]]). Full rationale for the older choices: [[implementation-research]] (note its §1.1 MATLAB backend is superseded by PLECS).

## 1. Stack

| Component | Choice | License | Why |
|---|---|---|---|
| **Simulator (sole evidence)** | PLECS 4.8 Standalone (XML-RPC/JSON-RPC + `plecs-mcp`) | commercial (installed) | native PMSM/IM + FOC; scriptable; PE-MAS prior art. [[plecs-harness]] |
...
| **Optimizer** | scipy DE / pyswarm PSO / Ax-BoTorch or scikit-optimize BO | BSD/MIT | stage ③. [[design-loop]] |
| **RAG** | PaperQA2 over LanceDB | Apache 2.0 | citation-grounded, LiteLLM-native. [[knowledge-rag]] |
| **Memory** | SQLite (episodic/procedural) + LanceDB (semantic) | PD + Apache 2.0 | [[memory]] |
| **Consensus** | DRCY multi-run (k=3 + separate reconcile model) | — | [[guardrails-and-evidence]] |
...
**Token discipline is architectural, not incidental:** summarize-before-LLM (invariant #3), the optimizer replaces LLM re-guessing with tool search (no LLM tokens in stage ③), and iteration traces are compacted ([[memory]]). These three are what keep $/design bounded — measure it ([[evaluation-and-benchmark]]).

← [[ai-agent-mas-plan]] | [[implementation-research]] | [[agent-frameworks-2026-currency]]
