...
**Relevance:** For research workflows, longer max_turns (50-100) accommodates multi-step simulation-analysis cycles, versus 10-30 for coding tasks.
...
**Relevance:** Skills accumulate domain knowledge. Simulation workflows, topology selection heuristics, MATLAB debugging procedures, and loss analysis methodologies can all be encoded and improved over repeated use.
...
**Relevance:** PostToolUse hooks automate output validation. After simulation: verify output file creation, check efficiency bounds (0-100%), validate waveform dimensions, detect NaN/inf values. Catches garbage results before they pollute downstream analysis.
...
**Relevance:** Granular permissions enable safe MATLAB execution. `Bash(matlab *)` for scripts, `Bash(simulink *)` for model compilation, `Write(*.m)` for script creation, while blocking dangerous operations like `Bash(rm *)` or `Bash(sudo *)`.
...
**Relevance:** MCP is the cleanest integration pattern for external tools like MATLAB. A MATLAB MCP server wrapping the Engine API provides persistent sessions, structured tool definitions, and language-agnostic access.
...
**Relevance:** Context files persist project configuration. A power electronics project file would hold MATLAB paths, component library locations, simulation baseline references, and modeling conventions.
...
**Relevance:** Schema-validated output ensures simulation results are structurally correct. An example schema enforces efficiency bounds (0-100%) and non-negative losses/THD/voltage stress, rejecting invalid results before downstream consumption.

## Pattern 9: Checkpoint/Rollback (Hermes, Claude Code)

**Used by:** Hermes Agent (`/rollback`, `/snapshot`), Claude Code (`/rewind`)

**Relevance:** Checkpoints enable safe exploration of simulation parameter spaces. Checkpoint before risky parameter changes that could break models; failed simulations roll back to known-good state.
