...
- **[[design-loop-architecture]]** (key finding) — across AnalogSAGE, PHIA, PE-GPT, Multi-Agent LLM Control, the design stage is **topology → refine → parameter-optimize** with an *explicit numerical optimizer* (DE/PSO/BO) over the simulator. The plan had no optimizer (gap G-A).
- **[[agentic-workflow-patterns]]** — the canonical 2026 pattern catalog mapped to SRTP; names the evaluator-optimizer loop the plan uses un-named and the exclusions (no autonomous agents, no learned routing, no debate, no recursion).
- New source captures: **[[liu-2026-iet-llm-power-converter-framework]]**, **[[context-engineering-2026]]**.
