...
- **Readback requirement — CORRECTED 2026-07-19 (blocker cleared).** The earlier "Values only from top-level Outport" claim is **false for PLECS 4.8 here**: `plecs.simulate` returns **empty** `{Time, Values}` even with a top-level `Output` wired to a live signal (verified on a minimal sine model and the real 2L-VSI model). **Every template must instead route each summarized signal (§4), muxed, into a `ToFile` block** (`FileType=1` CSV, `WriteSignalNames=1`, `SampleTime=-1`, absolute `Filename`); the summarizer reads that CSV from disk (`numpy`). No header row is written. **Edit→close→reload** (re-loading an open model is stale). This is the concrete path from "model runs" to "model yields evidence" — see [[procedure-simulation-and-validation]] §1 and memory `plecs-readback-tofile`.
