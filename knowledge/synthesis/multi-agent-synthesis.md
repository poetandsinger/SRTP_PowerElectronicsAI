...
### Key Finding
**Flat delegation (one level of subagents) covers 90% of research workflows.** Deep recursion (subagent spawns subagent) adds complexity without proportional benefit for power electronics research. Default to one-level orchestration: an orchestrator dispatching to N specialists.
...
**Key insight:** Multi-perspective debate before simulation is cheaper than running competing simulations. The debate surfaces disagreement early — before spending compute.
...
**Source:** arXiv:2502.11133v1 [[masrouter-2025-llm-routing]]
...
**Implication for SRTP:** Routing IS a recognized, solvable problem — MasRouter shows learned routing outperforms static routing. This partially addresses the red-team concern that "orchestrator routing is brittle."
...
**Source:** arXiv:2406.14228v3, accepted at **NAACL 2025** (peer-reviewed) [[evoagent-2025-evolutionary-delegation]]
...
> **A research pass on 2026-07-10 uncovered 10+ new sources that strengthen and refine this synthesis. The key findings are summarized here. Full source notes in `sources/ai-agents/`.**

### 5.1 Hybrid LangGraph-CrewAI Validated (IEEE Access, April 2026)

Khan et al. (2026) [[hybrid-langgraph-crewai-2026-ieee]] published a systematic evaluation in IEEE Access showing that a **hybrid LangGraph-CrewAI architecture** achieves:
...
- See [[langgraph-production-gaps-2026-diagrid]]
...
A multi-agent LLM system with a fine-tuned domain model designed a 2nm nanosheet FET in **4.2 hours vs 7.1 days** (40× speedup). Peer-reviewed at DATE 2026. [[agentic-tcad-2026-date]]

**Impact on our architecture:** This is the strongest evidence that multi-agent LLM systems can accelerate hardware design. The generate-verify-optimize loop (LLM→TCAD→analyze→iterate) is structurally identical to our proposed LLM→MATLAB→analyze→iterate loop.

### 5.5 DRCY: Production Multi-Agent Schematic Review with Multi-Run Consensus

AllSpice's DRCY system [[drcy-2026-allspice-mas-review]] uses **multi-run consensus** (k independent review agents + consensus reconciliation) deployed at Fortune 500 companies. This is a reliability pattern we should adopt for our Reviewer Agent.

### 5.6 ltspice-mcp: 51 SPICE Tools for LLM Agents

An MCP server [[ltspice-mcp-2026]] providing 51 SPICE simulation tools directly to LLM agents — circuit creation, transient/AC/DC analysis, Monte Carlo, THD, Bode plots. Compatible with Claude Code natively. This enables device-level circuit verification as a complement to MATLAB/Simulink system-level simulation.

### 5.7 Integration Architecture Published

A comprehensive integration architecture [[traction-inverter-mas-integration]] bridges the ee/ and cs/ domains with a 7-agent system specifically for traction inverter design. Key additions: Thermal Agent, Topology Agent, Multi-Physics Coordinator, 8 evidence gates, 7 domain guardrails, confidence-ranked claims (C1-C5), and a Phase 0 A/B test to validate multi-agent vs single-agent.
